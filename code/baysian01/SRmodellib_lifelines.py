...
def getSr(theta, n=25000,nsteps=4500,t_end=110, external_hazard = np.inf, time_step_multiplier=1):

    eta = theta[0]
    beta = theta[1]
    epsilon = theta[2]
    xc = theta[3]
    sim = SR_lf(eta=eta,beta=beta,epsilon=epsilon,xc=xc,kappa=0.5,npeople=n,nsteps=nsteps,t_end=t_end,external_hazard=external_hazard,time_step_multiplier=time_step_multiplier)
    return sim